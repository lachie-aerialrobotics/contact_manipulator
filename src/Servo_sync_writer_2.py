...
from contact_manipulator.cfg import ServoConfig
...
if __name__ == '__main__':
    ...
    servo_angle_pub = rospy.Publisher(robot_name+'/servo_angles_detected', servo_angles, queue_size=1, tcp_nodelay=True) # servo angle publisher
    servo_angle_sub = rospy.Subscriber(robot_name+'/servo_angles_setpoint', servo_angles, s.position_callback, tcp_nodelay=True) #target angle subscriber
    servo_current_sub = rospy.Subscriber(robot_name+'/servo_current_lims', servo_angles, s.current_callback, tcp_nodelay=True) #current limit subscriber
